...
<div class="page-container">
    <p class="f-20 text-success">欢迎使用ForLove.admin <span class="f-14">v1.0</span>后台！</p>
    <p>登录次数：<?php echo $number; ?> </p>
    <p>上次登录IP：<?php echo $login_ip; ?> 上次登录时间：<?php echo $login_time; ?></p>
    <table class="table table-border table-bordered table-bg">
        ...
        <tr class="text-c">
            <td>总数</td>
            <td id="countUser">0</td>
    				<td id="countMember">0</td>
    				<td id="countActivity">0</td>
    				<td id="countFeedback">0</td>
    				<td id="countAdmin">0</td>
        </tr>
<!--        <tr class="text-c">-->
<!--            <td>今日</td>-->
<!--            <td>0</td>-->
...
<script>
    ...
    function check() {
     $.ajax({
        type : 'get',
        url  : '<?php echo site_url('admin/Index/messageStatistics');?>',
        data : 'status='+1+'&member='+1,
        dataType : 'json',
        success : function (data) {
          $('#countUser').html(data.countUser);
          $('#countMember').html(data.countMember);
          $('#countActivity').html(data.countActivity);
          $('#countFeedback').html(data.countFeedback);
          $('#countAdmin').html(data.countAdmin);
        }
     });
